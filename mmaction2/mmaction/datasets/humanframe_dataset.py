...
class HumanframeDataset(BaseDataset):
    ...
    def __init__(self,
                 ...
                 rgb_data_prefix="../data/kinetics400/ori/",
                 seg_data_prefix="../data/kinetics400/seg/",
                 rgb_filename_tmpl='{:06}.jpg',
                 seg_filename_tmpl='{:06}.png',
                 **kwargs):
        self.filename_tmpl = filename_tmpl
        self.with_offset = with_offset
        assert data_prefix==None or data_prefix==''
        self.rgb_data_prefix= rgb_data_prefix
        self.seg_data_prefix= seg_data_prefix
        self.rgb_filename_tmpl= rgb_filename_tmpl
        self.seg_filename_tmpl= seg_filename_tmpl
        
        
        pipeline_seg = []
        for line in pipeline:
            if line['type'] == 'Normalize':
                line = line.copy()
                line['mean'] = [0,0,0]
                line['std'] = [255, 255, 255]
                pipeline_seg.append(line.copy())
            else:
                pipeline_seg.append(line.copy())
        
        self.pipeline_seg = Compose(pipeline_seg)
        super().__init__(
            ann_file,
            pipeline,
            data_prefix,
    ...
    def prepare_test_frames(self, idx):
        ...
        results['frame_dir'] = self.rgb_data_prefix + results['frame_dir']
        results['filename_tmpl'] = self.rgb_filename_tmpl
        results['modality'] = self.modality
        results['start_index'] = self.start_index
        ...
        results = copy.deepcopy(self.video_infos[idx])
        results['frame_dir'] = self.seg_data_prefix + results['frame_dir']
        results['filename_tmpl'] = self.seg_filename_tmpl
        results['modality'] = self.modality
        results['start_index'] = self.start_index
        segs = self.pipeline(results)
        
        if imgs['imgs'].dtype == torch.uint8:
            segs['imgs'] = (segs['imgs'] > 128).type(torch.uint8)
        else:
            segs['imgs'] = (segs['imgs'] > 0).type(torch.uint8)
        
        output = {}
        
        
        
        output['imgs'] = imgs['imgs'] * segs['imgs']
        
        # print(output['imgs'].dtype)
        if imgs['imgs'].dtype == torch.uint8:
            output['imgs'] += ((1-segs['imgs']) * np.ones_like(imgs['imgs']) * np.array([123.675, 116.28, 103.53]).reshape([1,3,1,1])).type(torch.uint8)
        # print(output['imgs'].dtype)
        
        output['label'] = imgs['label']
        return output